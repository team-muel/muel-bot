...
CREATE TABLE public.weave_nodes (
  ...
  source_kind text NOT NULL CHECK (source_kind IN (
    ...
    'subscription_signal',    -- 기본 알림 (메타 + 텍스트 요약)
    'community_video',        -- 커뮤니티 영상 (subscription_signal 중 영상 종류 분리)
    'community_post',         -- 커뮤니티 게시글 (이슈/공지/장문 글 — 게시판 식)
    'user_memo',              -- /메모 add 직접
...
## 외부 참고: DreamWeave (`scssm585536-netizen/dreamweave`)

비슷한 *꿈 직조기* 패턴의 오픈 저장소. 우리에 *맞는 부분만* 차용, *우리 정체성과 안 맞는 부분*은 제외.

**차용 (Adopt)**:
- **R3F 3D 그래프 시각화** (`@react-three/fiber + drei + three`). 우리 `WeaveCanvas.tsx` + `/force` 가 이미 같은 스택 — 멀티 source 노드 그래프로 확장.
- **AI 보조 (요약/태깅/연결 제안)** — 텍스트는 우리 Gemini lane registry 그대로. dreamweave 의 *입력 → AI 생성 → 그래프 노드* 흐름을 우리 메모/노트에 적용.
- **PDF / 이미지 export** (`html2canvas + jspdf`) — 사용자 본인의 노드/그래프를 외부로 가져갈 수 있게. *내 나무*의 export 기능 후속.
- **SSR Supabase 인증 패턴** (`@supabase/ssr`) — visibility 정책의 RLS 보조.

**제외 (Skip)**:
- **Stripe 결제** — 우리는 Toss 사용 중이고 *내부 도구* 정체성에 과금 우선순위 낮음.
- **fal.ai 이미지 생성** — Muel 핵심 기능 아님. 비용/스코프 증가. 옵션 보류.
- **OpenAI 위주** — 우리 modelRegistry (Gemini lane) 유지.

## 두 페르소나: 개인 Muel ↔ 커뮤니티 Muel

사용자 결정 (2026-06-05): 데이터 가시성과 서비스 범위가 두 *Muel 페르소나* 로 분리.

| 차원 | 개인 Muel (private) | 커뮤니티 Muel (community) |
|---|---|---|
| 노출 | 본인만 | 길드 멤버 모두 |
| 입력 surface | DM, /메모, /weave 내 나무 | 서버 채널 멘션, hub, /weave 나무위키 view |
| visibility 기본값 | `private` | `community` |
| 데이터 종류 | user_memo, auto_memo, 개인 AI-Q 리포트 | community_video, community_post, dream (공개), subscription_signal |
| Muel 의 톤 | 짧고 dense, 개인 정보 다룸 | 보조자, *멤버 작성 정리/연결 제안* |

*승격* (private → community) UX 는 보류 — 결정 이후 ADR 갱신.

## 나무위키 / 게시판 view (보류, 가능성 있음)

ADR-002 1차안은 *임베딩 유사도 그래프* (3D R3F) 가 일차 view. 사용자가 *카드/스레드 형태의 게시글 피드/리스트 view* 도 같이 가능성 언급 (2026-06-05).

후보 view (community visibility 노드만 대상):
- **그래프 view** — R3F 3D, 임베딩 유사도 엣지. *지식의 가지* 시각화.
- **피드 view** — 최신순 카드 그리드, *나무위키 페이지 리스트*.
- **스레드 view** — community_post 위주의 *게시판 식*. 답글/태그.

세 view 가 *같은 데이터 (weave_nodes)* 의 *다른 표현*. 결정/디자인은 Phase 1 이후.

## 변경 이력

- 2026-06-05: 초안. 사용자 결정 (리서치/구독 자산화 + Weave 통합) 반영.
- 2026-06-05 (갱신): 사용자 확인 (1-5)
  - PR muel-tree#13 close 결정 (랜딩 톤 ≠ bot 톤, 의도).
  - dreamweave 저장소 차용 / 제외 정리.
  - source_kind 에 `community_video` + `community_post` 분리 추가 (이전 subscription_signal 한 종에서 분화).
  - 두 페르소나 (개인 / 커뮤니티) 명시.
  - 게시판/피드 view 가능성 언급 (그래프 외).
  - private → community 승격 UX = 보류.
