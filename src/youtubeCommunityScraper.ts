...
const deriveCommunityPostTitle = (content: string, fallback: string): string => {
  ...
  if (!normalized) {
    return truncate(fallback || 'YouTube community post', 180);
  }
...
export const scrapeLatestCommunityPostByUrl = async (
  ...
  const title = deriveCommunityPostTitle(content, 'YouTube community post');
  ...
  return {
    id: postId,
    title: title || 'YouTube community post',
    content,
