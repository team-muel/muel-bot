...
function runTests() {
  ...
  assert('YouTube post with image -> embed image included', Boolean(res1.embeds?.[0] && (res1.embeds[0] as any).data.image?.url === 'https://example.com/image.jpg'));
  ...
  assert('YouTube post without image -> works correctly', Boolean(res2.embeds?.[0] && !(res2.embeds[0] as any).data.image));
